...
const TradingJournalDrawer = ({ isOpen, onClose, trade }) => {
  ...
  const handleCommit = () => {
    if (!note && !sentiment) {
      alert("Please add at least a note or sentiment tag.");
      return;
    }
    ...
    setTimeout(() => {
      const journalEntry = {
        ...
        externalLinks: { chart: chartLink, hash: txHash },
        timestamp: new Date().toISOString(),
      };

      console.log("Saving to Database:", journalEntry);
      setIsSaving(false);
      onClose();
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="space-y-2">
            ...
            <div className="bg-[#050505] p-4 rounded-xl border border-[#2A2F3A] text-xs text-gray-400 leading-relaxed font-mono">
              "Trade aligned with 4H bullish divergence on RSI. Volume profile
              indicated strong support at $124. Exit was slightly premature
              relative to Fibonacci extension levels ($145 target)."
            </div>
          </div>
          <div className="space-y-4">
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
          ...
          <div className="pt-2 pb-8">
            <button
              ...
              disabled={isSaving}
              className={`w-full py-4 font-black uppercase rounded-xl transition shadow-[0_0_20px_rgba(255,255,255,0.2)] tracking-widest text-xs flex justify-center items-center gap-2 ${
                isSaving
